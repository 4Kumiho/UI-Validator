...
class SettingsScreen(Screen):
    ...
    open_menu_key = StringProperty("")  # Obbligatorio - caricato dal JSON
    refresh_screenshot_key = StringProperty("")  # Opzionale
    end_input_key = StringProperty("")  # Opzionale
    end_designer_key = StringProperty("")  # Opzionale
    ...
    def _build_key_combo(self, selected_dict: dict, allow_empty: bool = False) -> str:
        ...
        if not parts:
            return "" if allow_empty else "esc"
    ...
    def _show_key_popup(self, field_type: str, selected_dict: dict, current_value: str):
        ...
        for key in self.AVAILABLE_KEYS:
            ...
            def make_callback(k, sd):
                def on_checkbox_change(checkbox, value):
                    ...
                    error_label.opacity = 0  # Hide error when user selects something
        ...
        # Buttons at bottom
        btn_layout = BoxLayout(size_hint_y=None, height=dp(50), spacing=dp(15), padding=dp(10))
        ...
        def on_done_press(btn):
            # Check if field is required and no keys selected
            if is_required and not any(selected_dict.values()):
                error_label.opacity = 1
            else:
                popup.dismiss()

        done_btn.bind(on_press=on_done_press)
    ...
    def _update_property(self, field_type: str, selected_dict: dict):
        ...
        is_required = field_type == 'open_menu'
        new_value = self._build_key_combo(selected_dict, allow_empty=not is_required)
    ...
    def set_defaults(self):
        ...
        self.refresh_screenshot_key = ""
        self.end_input_key = ""
        self.end_designer_key = ""
    ...
    def save_settings(self):
        ...
        # Validate that open_menu_key is required
        if not self.open_menu_key or self.open_menu_key.strip() == "":
            print("[ERROR] open_menu_key is required!")
            return
        ...
        try:
            ...
            print(f"  refresh_screenshot_key: {self.refresh_screenshot_key or 'null'}")
            print(f"  end_input_key: {self.end_input_key or 'null'}")
            print(f"  end_designer_key: {self.end_designer_key or 'null'}")
            self.go_back()
        except Exception as e:
